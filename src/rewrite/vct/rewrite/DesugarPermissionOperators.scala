...
import vct.col.rewrite.DesugarPermissionOperators.{
  FramedArraySubscriptBlame,
  FramedPointerDerefBlame,
  PredicateValueError,
}
...
case object DesugarPermissionOperators extends RewriterBuilder {
  ...
  override def desc: String =
    "Desugar assorted syntactic sugar for permission predicates."

  case class FramedArraySubscriptBlame(blame: Blame[ArrayLocationError])
  ...
  case class FramedPointerDerefBlame(blame: Blame[PointerLocationError])
  ...
  case class PredicateValueError(loc: Location[_]) extends UserError {
    ...
    override def text: String =
      loc.o.messageInContext(
        "The predicate has a location but does not point to a value."
      )
  }

}

case class DesugarPermissionOperators[Pre <: Generation]()
    extends Rewriter[Pre] {

  def extractValueFromLocation(loc: Location[Pre]): Expr[Pre] = {
    loc match {
      case HeapVariableLocation(field) =>
        DerefHeapVariable(field)(PointsToDeref)(loc.o)
      case FieldLocation(obj, field) => Deref(obj, field)(PointsToDeref)(loc.o)
      case ModelLocation(obj, field) =>
        ModelDeref(obj, field)(PointsToDeref)(loc.o)
      case SilverFieldLocation(obj, field) =>
        SilverDeref(obj, field)(PointsToDeref)(loc.o)
      case node @ ArrayLocation(array, subscript) =>
  ...
  override def dispatch(e: Expr[Pre]): Expr[Post] = {
    ...
    e match {
      case PointsTo(loc1, perm1, value1) =>
        val (loc, perm, value) =
          (dispatch(loc1), dispatch(perm1), dispatch(value1))
      ...
      case ValidMatrix(mat1, dim01, dim11) =>
        val (mat, dim0, dim1) =
          (dispatch(mat1), dispatch(dim01), dispatch(dim11))
        (mat !== Null()) && (Length(mat)(FramedArrLength) === dim0) &* starall(
          IteratedArrayInjective,
          TInt(),
          row =>
          ...
          row =>
            (const(0) <= row && row < dim0) ==>
              (Length(ArraySubscript(mat, row)(FramedArrIndex))(
                FramedArrLength
              ) === dim1),
        ) &* forall(
          TInt(),
          row0 =>
            forall(
              TInt(),
              row1 =>
                (const(0) <= row0 && row0 < dim0 && const(0) <= row1 &&
                  row1 < dim0) ==>
                  ((ArraySubscript(mat, row0)(FramedArrIndex) ===
                    ArraySubscript(mat, row1)(FramedArrIndex)) ==>
                    (row0 === row1)),
            ),
        )
