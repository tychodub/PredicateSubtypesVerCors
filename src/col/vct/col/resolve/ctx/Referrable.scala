...
/** Collection of all things that can be cross-referenced in the AST. This
  * includes all declarations, indices at multi-declarations, references to
  * implicitly defined constructs, built-in fields and methods, etc. Referrables
  * are briefly used while resolving the AST, and are almost immediately
  * replaced by instances of [[vct.col.ref.Ref]]. See also
  * [[vct.col.resolve.ResolveTypes]] and [[vct.col.resolve.ResolveReferences]].
  * @tparam G
  ...
  def name: String =
    this match {
      ...
      case RefCGlobalDeclaration(decls, initIdx) =>
        C.nameFromDeclarator(decls.decl.inits(initIdx).decl)
      ...
      case RefCPPFunctionDefinition(decl) =>
        CPP.nameFromDeclarator(decl.declarator)
      case RefCPPGlobalDeclaration(decls, initIdx) =>
        CPP.nameFromDeclarator(decls.decl.inits(initIdx).decl)
      case RefCPPCustomType(typeName) => typeName
      case RefCPPLambdaDefinition(decl) => ""
      case RefCPPLocalDeclaration(decls, initIdx) =>
        CPP.nameFromDeclarator(decls.decl.inits(initIdx).decl)
      ...
      case RefOpenCLVectorMembers(idxs) =>
        "s" ++ idxs.map { i => f"$i%x" }.mkString
...
case object Referrable {
  def from[G](decl: Declaration[G]): Seq[Referrable[G]] =
    Seq[Referrable[G]](decl match {
      ...
      case decl: CStructMemberDeclarator[G] =>
      ...
      case decl: CPPGlobalDeclaration[G] =>
        return decl.decl.inits.indices.map(RefCPPGlobalDeclaration(decl, _))
      ...
      case decl: JavaFields[G] =>
        return decl.decls.indices.map(RefJavaField(decl, _))
      ...
      case decl: CLocalDeclaration[G] =>
        return decl.decl.inits.indices.map(RefCLocalDeclaration(decl, _))
      case decl: CPPLocalDeclaration[G] =>
        return decl.decl.inits.indices.map(RefCPPLocalDeclaration(decl, _))
      case decl: JavaLocalDeclaration[G] =>
        return decl.decls.indices.map(RefJavaLocalDeclaration(decl, _))
      ...
      case decl: BipTransitionSynchronization[G] =>
        RefBipTransitionSynchronization(decl)
  ...
  def originName(decl: Declaration[_]): String =
    decl.o.find[SourceName] match {
      case Some(SourceName(name)) => name
      case _ => throw NameLost(decl.o)
    }

  def originNameOrEmpty(decl: Declaration[_]): String =
    decl.o.find[SourceName] match {
      case Some(SourceName(name)) => name
      case _ => ""
    }
...
sealed trait SpecTypeNameTarget[G]
    extends JavaTypeNameTarget[G]
    with CTypeNameTarget[G]
    with CPPTypeNameTarget[G]
...
sealed trait SpecNameTarget[G]
    extends CNameTarget[G]
    with CPPNameTarget[G]
    with JavaNameTarget[G]
    with PVLNameTarget[G]
...
sealed trait SpecDerefTarget[G]
    extends CDerefTarget[G]
    with CPPDerefTarget[G]
    with JavaDerefTarget[G]
    with PVLDerefTarget[G]
...
case class RefCTranslationUnit[G](decl: CTranslationUnit[G])
    extends Referrable[G]
case class RefCParam[G](decl: CParam[G])
    extends Referrable[G] with CNameTarget[G]
case class RefOpenCLVectorLiteralCInvocationTarget[G](
    size: BigInt,
    innerType: Type[G],
) extends CInvocationTarget[G]
case class RefCFunctionDefinition[G](decl: CFunctionDefinition[G])
    extends Referrable[G]
    with CNameTarget[G]
    with CInvocationTarget[G]
    with ResultTarget[G]
case class RefCGlobalDeclaration[G](decls: CGlobalDeclaration[G], initIdx: Int)
    extends Referrable[G]
    with CNameTarget[G]
    with CInvocationTarget[G]
    with ResultTarget[G]
case class RefCLocalDeclaration[G](decls: CLocalDeclaration[G], initIdx: Int)
    extends Referrable[G] with CNameTarget[G]
case class RefCPPTranslationUnit[G](decl: CPPTranslationUnit[G])
    extends Referrable[G]
case class RefCPPParam[G](decl: CPPParam[G])
    extends Referrable[G] with CPPNameTarget[G]
case class RefCPPFunctionDefinition[G](decl: CPPFunctionDefinition[G])
    extends Referrable[G]
    with CPPNameTarget[G]
    with CPPInvocationTarget[G]
    with ResultTarget[G]
case class RefCPPLambdaDefinition[G](decl: CPPLambdaDefinition[G])
    extends Referrable[G] with CPPInvocationTarget[G] with CPPTypeNameTarget[G]
case class RefCPPCustomType[G](typeName: String)
    extends Referrable[G] with CPPTypeNameTarget[G]
case class RefCPPGlobalDeclaration[G](
    decls: CPPGlobalDeclaration[G],
    initIdx: Int,
) extends Referrable[G]
    with CPPNameTarget[G]
    with CPPInvocationTarget[G]
    with CPPDerefTarget[G]
    with ResultTarget[G]
case class RefCPPLocalDeclaration[G](
    decls: CPPLocalDeclaration[G],
    initIdx: Int,
) extends Referrable[G] with CPPNameTarget[G]
case class RefSYCLAccessMode[G](decl: SYCLAccessMode[G])
    extends Referrable[G] with CPPNameTarget[G]
case class RefSYCLConstructorDefinition[G](typ: SYCLTConstructableClass[G])
    extends Referrable[G] with CPPNameTarget[G] with CPPInvocationTarget[G]
case class RefJavaNamespace[G](decl: JavaNamespace[G]) extends Referrable[G]
case class RefUnloadedJavaNamespace[G](names: Seq[String])
    extends Referrable[G] with JavaNameTarget[G] with JavaDerefTarget[G]
case class RefTypeDef[G](decl: CGlobalDeclaration[G])
    extends Referrable[G] with CTypeNameTarget[G] with CNameTarget[G]
case class RefCStruct[G](decl: CGlobalDeclaration[G])
    extends Referrable[G]
    with CStructTarget[G]
    with CNameTarget[G]
    with CDerefTarget[G]
...
case class RefJavaClass[G](decl: JavaClassOrInterface[G])
    extends Referrable[G]
    with JavaTypeNameTarget[G]
    with JavaNameTarget[G]
    with JavaDerefTarget[G]
    with ThisTarget[G]
case class RefSilverField[G](decl: SilverField[G]) extends Referrable[G]
case class RefSimplificationRule[G](decl: SimplificationRule[G])
    extends Referrable[G]
case class RefAxiomaticDataType[G](decl: AxiomaticDataType[G])
    extends Referrable[G]
    with SpecTypeNameTarget[G]
    with SpecNameTarget[G]
    with JavaDerefTarget[G]
case class RefFunction[G](decl: Function[G])
    extends Referrable[G] with SpecInvocationTarget[G] with ResultTarget[G]
case class RefProcedure[G](decl: Procedure[G])
    extends Referrable[G] with SpecInvocationTarget[G] with ResultTarget[G]
case class RefVeSUVMainMethod[G](decl: VeSUVMainMethod[G])
    extends Referrable[G] with SpecInvocationTarget[G] with ResultTarget[G]
...
case class RefClass[G](decl: Class[G])
    ...
    with SpecNameTarget[G]
    with ThisTarget[G]
case class RefModel[G](decl: Model[G])
    extends Referrable[G]
    with SpecTypeNameTarget[G]
    with ThisTarget[G]
...
case class RefJavaField[G](decls: JavaFields[G], idx: Int)
    extends Referrable[G] with JavaNameTarget[G] with JavaDerefTarget[G]
case class RefJavaLocalDeclaration[G](decls: JavaLocalDeclaration[G], idx: Int)
    extends Referrable[G] with JavaNameTarget[G]
case class RefJavaConstructor[G](decl: JavaConstructor[G])
    extends Referrable[G] with JavaConstructorTarget[G]
case class RefJavaMethod[G](decl: JavaMethod[G])
    extends Referrable[G] with JavaInvocationTarget[G] with ResultTarget[G]
case class RefJavaParam[G](decl: JavaParam[G])
    extends Referrable[G] with JavaNameTarget[G]
case class RefJavaAnnotationMethod[G](decl: JavaAnnotationMethod[G])
    extends Referrable[G] with JavaInvocationTarget[G] with ResultTarget[G]
case class RefInstanceFunction[G](decl: InstanceFunction[G])
    extends Referrable[G] with SpecInvocationTarget[G] with ResultTarget[G]
case class RefInstanceMethod[G](decl: InstanceMethod[G])
    extends Referrable[G] with SpecInvocationTarget[G] with ResultTarget[G]
case class RefInstanceOperatorMethod[G](decl: InstanceOperatorMethod[G])
...
case class RefVariable[G](decl: Variable[G])
    extends Referrable[G] with SpecNameTarget[G] with SpecTypeNameTarget[G]
...
case class RefParInvariantDecl[G](decl: ParInvariantDecl[G])
    extends Referrable[G]
case class RefADTAxiom[G](decl: ADTAxiom[G]) extends Referrable[G]
case class RefADTFunction[G](decl: ADTFunction[G])
    extends Referrable[G] with SpecInvocationTarget[G]
case class RefModelField[G](decl: ModelField[G])
    extends Referrable[G] with SpecDerefTarget[G] with SpecNameTarget[G]
case class RefModelProcess[G](decl: ModelProcess[G])
    extends Referrable[G] with SpecInvocationTarget[G]
case class RefModelAction[G](decl: ModelAction[G])
    extends Referrable[G] with SpecInvocationTarget[G]
case class RefPVLConstructor[G](decl: PVLConstructor[G])
    extends Referrable[G] with PVLConstructorTarget[G]
case class RefJavaBipStatePredicate[G](state: String, decl: JavaAnnotation[G])
    extends Referrable[G] with JavaBipStatePredicateTarget[G]
case class RefJavaBipGuard[G](decl: JavaMethod[G])
    extends Referrable[G] with JavaNameTarget[G]
...
case class RefBipPortSynchronization[G](decl: BipPortSynchronization[G])
    extends Referrable[G]
case class RefBipStatePredicate[G](decl: BipStatePredicate[G])
    extends Referrable[G]
case class RefBipTransition[G](decl: BipTransition[G]) extends Referrable[G]
case class RefBipTransitionSynchronization[G](
    decl: BipTransitionSynchronization[G]
) extends Referrable[G]
...
case class RefPVLEndpoint[G](decl: PVLEndpoint[G])
    extends Referrable[G] with PVLNameTarget[G]
...
case class RefProverType[G](decl: ProverType[G])
    extends Referrable[G] with SpecTypeNameTarget[G]
case class RefProverFunction[G](decl: ProverFunction[G])
    extends Referrable[G] with SpecInvocationTarget[G]

case class BuiltinField[G](f: Expr[G] => Expr[G])
    extends Referrable[G] with SpecDerefTarget[G]
case class BuiltinInstanceMethod[G](f: Expr[G] => Seq[Expr[G]] => Expr[G])
    extends Referrable[G] with SpecInvocationTarget[G]

case class PVLBuiltinInstanceMethod[G](f: Expr[G] => Seq[Expr[G]] => Expr[G])
    extends Referrable[G] with PVLInvocationTarget[G]

case class ImplicitDefaultJavaConstructor[G](cls: JavaClass[G])
    extends Referrable[G] with JavaConstructorTarget[G]
case class ImplicitDefaultPVLConstructor[G](cls: Class[G])
    extends Referrable[G] with PVLConstructorTarget[G]

case class ImplicitDefaultJavaBipStatePredicate[G](state: String)
    extends Referrable[G] with JavaBipStatePredicateTarget[G]
