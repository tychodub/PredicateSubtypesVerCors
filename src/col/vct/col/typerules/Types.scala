...
object Types {
  ...
  def leastCommonSuperType[G](left: Type[G], right: Type[G]): Type[G] =
    (left, right) match {
      ...
      case (TTuple(left), TTuple(right)) if left.size == right.size =>
        TTuple(left.zip(right).map { case (l, r) =>
          leastCommonSuperType(l, r)
        })
      case (TSeq(left), TSeq(right)) => TSeq(leastCommonSuperType(left, right))
      case (TSet(left), TSet(right)) => TSet(leastCommonSuperType(left, right))
      case (TVector(sizeLeft, left), TVector(sizeRight, right))
          if sizeLeft == sizeRight =>
        TVector(sizeLeft, leastCommonSuperType(left, right))()
      case (TBag(left), TBag(right)) => TBag(leastCommonSuperType(left, right))
      ...
      case (TMap(leftK, leftV), TMap(rightK, rightV)) =>
        ...
        if (leftK == rightK)
          TMap(leftK, leastCommonSuperType(leftV, rightV))
        else
          TAnyValue()
      ...
      case (left: TClass[G], right: TClass[G]) =>
        ...
        val shared = leftArrows.collect {
          case (_, sup) if rightArrows.exists { case (_, rsup) =>
                rsup == sup
              } =>
            sup
        }
        // We are not interested in types that occur above shared types
        val nonBottom = leftArrows.intersect(rightArrows).map {
          case (sub, sup) => sup
        }
      ...
      case (left, right)
          if TAnyValue().superTypeOf(left) && TAnyValue().superTypeOf(right) =>
        TAnyValue()
