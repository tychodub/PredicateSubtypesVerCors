...
case class CPPToCol[G](
  ...
  private def getPrependedNameDeclarator(
      declarator: CPPDeclarator[G]
  )(implicit o: Origin): CPPDeclarator[G] = {
    if (prependNamespace) {
      declarator match {
        case decl: CPPTypedFunctionDeclarator[G] =>
          CPPTypedFunctionDeclarator[G](
            decl.params,
            decl.varargs,
            getPrependedNameDeclarator(decl.inner),
          )
        case CPPName(name) =>
          CPPName[G]((currentNamespacePath.reverse :+ name).mkString("::"))
        case _ => declarator
      }
    } else { declarator }
  }
  ...
  def convert(
      implicit unit: TranslationUnitContext
  ...
  def convert(
      implicit declSeq: DeclarationseqContext
  ): Seq[GlobalDeclaration[G]] =
    declSeq match {
  ...
  def convert(
      implicit nsDef: NamespaceDefinitionContext
  ): Seq[GlobalDeclaration[G]] =
    nsDef match {
  ...
  def convert(
    ...
    funcDef match {
      case FunctionDefinition0(
        ...
        val convertedDeclarator = getPrependedNameDeclarator(
          convert(declarator)
        )
        prependNamespace = false
        val converted = withContract(
          maybeContract,
          contract =>
  ...
  def convert(implicit compoundStmnt: CompoundStatementContext): Statement[G] =
    compoundStmnt match {
      case CompoundStatement0(_, None, _) =>
        Scope(Nil, CPPLifetimeScope(Block(Seq())))
      case CompoundStatement0(_, Some(stmntSeq), _) =>
        Scope(Nil, CPPLifetimeScope(Block(convert(stmntSeq))))
    }
  ...
  def convert(
      implicit simpleDecl: SimpleDeclarationContext
  ): CPPDeclaration[G] =
    simpleDecl match {
      case SimpleDeclaration0(maybeContract, declSpecs, maybeInits, _) =>
        ...
        if (maybeInits.isDefined) {
          convert(maybeInits.get).foreach(init => {
            if (
              currentNamespacePath.nonEmpty && prependNamespace &&
              !init.decl.isInstanceOf[CPPTypedFunctionDeclarator[G]]
            ) {
  ...
  def convert(implicit exprStmnt: ExpressionStatementContext): Statement[G] =
    exprStmnt match {
      ...
      case ExpressionStatement0(Some(expr), _) =>
        Block(convert(expr).map(Eval(_)))
    }

  def convert(implicit exprs: OneOrMoreExpressionsContext): Seq[Expr[G]] =
    exprs match {
      case OneOrMoreExpressions0(expr, _, moreExprs) =>
        convert(expr) +: convert(moreExprs)
      case OneOrMoreExpressions1(expr) => Seq(convert(expr))
    }

  def convert(implicit expr: ExpressionContext): Expr[G] =
    expr match {
      case Expression0(pre, inner, post) =>
  ...
  def convert(
      implicit selectionStmnt: SelectionStatementContext
  ): Statement[G] =
      ...
      case IterationStatement2(
        ...
        val loopInit =
          init match {
            ...
            case ForInitStatement1(simpleDecl) =>
  ...
  def convert(
      ...
      targetNode: PointerMemberExpressionContext,
    ...
    PreAssignExpression(
      ...
      op match {
        ...
        case AssignmentOperator2(_) =>
          AmbiguousTruncDiv(target, value)(blame(expr))
        case AssignmentOperator3(_) =>
          AmbiguousTruncMod(target, value)(blame(expr))
  ...
  def convert(op: ImplicationOpContext, left: Expr[G], right: Expr[G])(
      implicit o: Origin
  ): Expr[G] =
    op match {
  ...
  def convert(implicit bracedInitList: BracedInitListContext): Expr[G] =
    bracedInitList match {
      case BracedInitList0(_, _) => ??(bracedInitList)
      case BracedInitList1(_, initList, _) => CPPLiteralArray(convert(initList))
      case BracedInitList2(_, initList, _, _) =>
        CPPLiteralArray(convert(initList))
    }
  ...
  def convert(implicit expr: PrimaryExpressionContext): Expr[G] =
    expr match {
      ...
      case PrimaryExpression3(_, inner, _) =>
        convert(inner) match {
          case Seq(i) => i
          case _ => ??(expr)
        }
      case PrimaryExpression4(inner) =>
        convert(inner) match {
          case CPPTypedefName(name, arg) =>
            CPPLocal(name, arg)(blame(expr))(origin(expr))
          case SYCLClassDefName(name, arg) =>
            CPPLocal("sycl::" + name, arg)(blame(expr))(origin(expr))
          case _ => ??(expr)
        }
  ...
  def convert(implicit idExpr: IdExpressionContext): CPPTypeSpecifier[G] =
    idExpr match {
  ...
  // Not supporting attribute specifiers
  def convert(
      implicit declSpecSeq: DeclSpecifierSeqContext
  ...
  def convert(
      implicit declSpec: DeclSpecifierContext
  ): Seq[CPPDeclarationSpecifier[G]] =
    declSpec match {
  ...
  def convert(
      implicit typeSpec: TypeSpecifierContext
  ...
  def convert(
      implicit typeSpec: TrailingTypeSpecifierContext
  ): Seq[CPPTypeSpecifier[G]] =
    typeSpec match {
  ...
  def convert(
      implicit typeSpec: SimpleTypeSpecifierContext
  ): Seq[CPPTypeSpecifier[G]] =
    typeSpec match {
      ...
      case SimpleTypeSpecifier0(Some(nestedNameSpec), typeName) =>
        convert(typeName) match {
          case CPPTypedefName(name, genericArgs) =>
            convert(nestedNameSpec).nestedName match {
              case s"sycl::$rest" =>
                Seq(SYCLClassDefName(rest + name, genericArgs))
              case other => Seq(CPPTypedefName(other + name, genericArgs))
            }
          case _ => ??(typeSpec)
        }
  ...
  def convert(
      implicit nestedNameSpec: NestedNameSpecifierContext
  ): CPPTypedefName[G] =
    nestedNameSpec match {
      case NestedNameSpecifier0(theTypeName, sep) =>
        convert(theTypeName) match {
          case name @ CPPTypedefName(_, Seq()) => name.appendName(sep)
          case _ => ??(theTypeName)
        }
      ...
      case NestedNameSpecifier3(sep) => CPPTypedefName(sep, Seq())
      case NestedNameSpecifier4(inner, id, sep) =>
        convert(inner) match {
          case name @ CPPTypedefName(_, Seq()) =>
  ...
  def convert(implicit namespaceName: NamespaceNameContext): CPPTypedefName[G] =
    namespaceName match {
      case NamespaceName0(OriginalNamespaceName0(id)) =>
        CPPTypedefName(convert(id), Seq())
      case NamespaceName1(NamespaceAlias0(id)) =>
        CPPTypedefName(convert(id), Seq())
    }
  ...
  def convert(implicit className: ClassNameContext): CPPTypedefName[G] =
    className match {
      case ClassName0(name) => CPPTypedefName(convert(name), Seq())
      case ClassName1(_) => ??(className)
    }

  def convert(implicit templateId: TemplateIdContext): CPPTypeSpecifier[G] =
    templateId match {
  ...
  def convert(
      implicit simpleTemplateId: SimpleTemplateIdContext
  ): CPPTypeSpecifier[G] =
    simpleTemplateId match {
      case SimpleTemplateId0(name, _, arg, _) =>
        CPPTypedefName(convert(name), Seq(convert(arg)))
      case SimpleTemplateId1(name, _, Some(args), _) =>
        CPPTypedefName(convert(name), convert(args))
      case SimpleTemplateId1(name, _, None, _) =>
        CPPTypedefName(convert(name), Seq())
    }

  def convert(
      implicit templateArgumentList: TemplateArgumentListContext
  ): Seq[CPPExprOrTypeSpecifier[G]] =
    templateArgumentList match {
      case TemplateArgumentList0(arg, None, _, argList) =>
        Seq(convert(arg)) ++ convert(argList)
      case TemplateArgumentList0(_, _, _, _) => ??(templateArgumentList)
      case TemplateArgumentList1(arg, None) => Seq(convert(arg))
      case TemplateArgumentList1(_, _) => ??(templateArgumentList)
    }

  // Only support constantExpression
  def convert(
      implicit templateArgument: TemplateArgumentContext
  ): CPPExprOrTypeSpecifier[G] =
    templateArgument match {
      case TemplateArgument0(theTypeId) =>
        convert(theTypeId) match {
          // Only support one typeSpec
          case Seq(typeSpec) => CPPExprOrTypeSpecifier(None, Some(typeSpec))
          case _ => ??(templateArgument)
        }
      case TemplateArgument1(constantExpr) =>
        CPPExprOrTypeSpecifier(Some(convert(constantExpr)), None)
      case TemplateArgument2(_) => ??(templateArgument)
    }

  def convert(implicit templateName: TemplateNameContext): String =
    templateName match { case TemplateName0(identifier) => convert(identifier) }

  // Do not support abstractDeclarator
  def convert(implicit theTypeId: TheTypeIdContext): Seq[CPPTypeSpecifier[G]] =
    theTypeId match {
      case TheTypeId0(typeSpecSeq, None) => convert(typeSpecSeq)
      case TheTypeId0(_, _) => ??(theTypeId)
    }

  // Do not support attributeSpecifierSeq
  def convert(
      implicit typeSpecSeq: TypeSpecifierSeqContext
  ): Seq[CPPTypeSpecifier[G]] =
    typeSpecSeq match {
      case TypeSpecifierSeq0(typeSpecs, None) => typeSpecs.flatMap(convert(_))
      case TypeSpecifierSeq0(_, _) => ??(typeSpecSeq)
    }
  ...
  def convert(
      implicit pointerDeclarator: PointerDeclaratorContext
  ): CPPDeclarator[G] =
  ...
  def convert(
      implicit pointer: PointerDeclaratorPrefixContext
  ): Seq[CPPAddressing[G]] =
    pointer match {
  ...
  def convert(
      implicit pointerOp: PointerOperatorWithDoubleStarContext
  ): Seq[CPPAddressing[G]] =
    pointerOp match {
  ...
  def convert(
      implicit pointerOp: PointerOperatorContext
  ): Seq[CPPAddressing[G]] =
    pointerOp match {
  ...
  def convert(
    ...
    noPointerDeclarator match {
      ...
      case NoPointerDeclarator2(innerDeclarator, _, None, _, None) =>
        CPPArrayDeclarator(None, convert(innerDeclarator))(blame(
          noPointerDeclarator
        ))
      case NoPointerDeclarator2(innerDeclarator, _, Some(constExpr), _, None) =>
        CPPArrayDeclarator(Some(convert(constExpr)), convert(innerDeclarator))(
          blame(noPointerDeclarator)
        )
  ...
  def convert(
      implicit paramsAndQuals: ParametersAndQualifiersContext
  ...
  def convert(
      implicit paramDeclClause: ParameterDeclarationClauseContext
  ): (Seq[CPPParam[G]], Boolean) =
  ...
  def convert(
      implicit paramDeclList: ParameterDeclarationListContext
  ): Seq[CPPParam[G]] =
    paramDeclList match {
  ...
  def convert(implicit declaratorId: DeclaratoridContext): CPPDeclarator[G] =
    declaratorId match {
      case Declaratorid0(None, idExpr) =>
        convert(idExpr) match {
  ...
  def convert(
      implicit unqualifiedId: UnqualifiedIdContext
  ): CPPTypeSpecifier[G] =
    unqualifiedId match {
      case UnqualifiedId0(templateId) => convert(templateId)
      case UnqualifiedId1(clangppId) =>
        CPPTypedefName(convert(clangppId), Seq())
  ...
  def convert(implicit qualifiedId: QualifiedIdContext): CPPTypeSpecifier[G] =
    qualifiedId match {
      case QualifiedId0(nestedNameSpec, None, id) =>
        convert(id) match {
          case CPPTypedefName(name, genericArgs) =>
            convert(nestedNameSpec).nestedName match {
              case s"sycl::$rest" => SYCLClassDefName(rest + name, genericArgs)
              case other => CPPTypedefName(other + name, genericArgs)
            }
          case _ => ??(qualifiedId)
        }
  ...
  def convert(
      implicit lambda: LambdaExpressionContext
  ...
  def convert(implicit decl: LambdaDeclaratorContext): CPPDeclarator[G] =
    decl match {
  ...
  def local(ctx: ParserRuleContext, name: String): Expr[G] =
    CPPLocal(name, Seq())(blame(ctx))(origin(ctx))
  ...
  def convert(implicit arg: ValArgContext): Variable[G] =
    arg match {
      case ValArg0(paramDecl) => {
        ...
        new Variable(
          CPP.typeOrReturnTypeFromDeclarator(param.specifiers, param.declarator)
        )(origin(arg).sourceName(CPP.nameFromDeclarator(param.declarator)))
      }
    }
        ...
        Seq(withContract(
          ...
          c =>
            withModifiers(
              modifiers,
              m => {
                val namedOrigin = origin(decl).sourceName(
                  (currentNamespacePath.reverse :+ convert(name)).mkString("::")
                )
                new Function(
                  convert(t),
  ...
  def evalOrNop(
      implicit expr: Option[OneOrMoreExpressionsContext]
  ): Statement[G] =
    expr match {
      case Some(exprs) =>
        Block(convert(exprs).map(e => Eval(e)(e.o)))(origin(exprs))
      case None =>
